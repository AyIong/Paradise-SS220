...
export function TguiSay() {
  ...
  const [currentPrefix, setCurrentPrefix] = useState<keyof typeof RADIO_PREFIXES | null>(null);
  ...
  function handleArrowKeys(direction: KEY.Up | KEY.Down): void {
    ...
    if (direction === KEY.Up) {
      ...
      const newContent = chat.isAtLatest() ? iterator.current() : chat.getIndex().toString();
  ...
  const theme =
    (lightMode && 'lightMode') || (currentPrefix && RADIO_PREFIXES[currentPrefix]) || channelIterator.current.current();
  ...
  return (
    <>
      ...
      >
        ...
        <textarea
          autoCorrect="off"
          className={classes(['textarea', `textarea-${theme}`, value.length > LineLength.Large && 'textarea-large'])}
          maxLength={maxLength}
          onInput={handleInput}
