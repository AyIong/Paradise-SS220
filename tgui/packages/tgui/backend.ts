...
export const backendCreatePayloadQueue = createAction('backend/createPayloadQueue');
export const backendDequeuePayloadQueue = createAction('backend/dequeuePayloadQueue');
export const backendRemovePayloadQueue = createAction('backend/removePayloadQueue');
...
export const backendMiddleware = (store) => {
  ...
  return (next) => (action) => {
    const { suspended, outgoingPayloadQueues } = selectBackend(store.getState());
    const { type, payload } = action;
...
const encodedLengthBinarySearch = (haystack: string[], length: number) => {
  ...
  while (low < high) {
    ...
    const substringLength = encodeURIComponent(haystack.slice(0, mid).join('')).length;
...
const chunkSplitter = {
  [Symbol.split]: (string: string) => {
    ...
    while (startIndex < length) {
      const cut = charSeq.slice(startIndex, endIndex < length ? endIndex : undefined);
      ...
      if (encodeURIComponent(cutString).length > 1024) {
        ...
        chunks.push(charSeq.slice(startIndex, splitIndex < length ? splitIndex : undefined).join(''));
...
export const sendAct = (action: string, payload: object = {}) => {
  ...
  const urlSize = Object.entries({
    ...
    (url, [key, value], i) => url + `${i > 0 ? '&' : '?'}${encodeURIComponent(key)}=${encodeURIComponent(value)}`,
    ''
  ).length;
  if (urlSize > 2048) {
