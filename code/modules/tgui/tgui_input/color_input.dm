...
/proc/tgui_input_color(mob/user, message, title, default = "#000000", timeout = 0, autofocus = TRUE, ui_state = GLOB.always_state)
	...
	var/datum/tgui_input_color/input = new(user, message, title, default, timeout, autofocus, ui_state)
...
/datum/tgui_input_color/ui_interact(mob/user, datum/tgui/ui)
	...
	if(!ui)
		ui = new(user, src, "ColorInputModal")
...
/datum/tgui_input_color/proc/set_choice(choice)
	src.choice = choice
